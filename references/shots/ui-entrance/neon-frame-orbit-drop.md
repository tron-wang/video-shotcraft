...
一句话: 细金线框先行描框后，镜头绕页面左→右弧线旋转，页面全部组件/文字**同帧**从空中往下贴合（同形软影同步收敛）——整体登场式的框内安放
适用: 单页 UI 的一次性隆重登场（与巡礼/逐区亮相相对）；暗场黑金调性段落的主视觉揭幕（口播片引用网页 / 产品页）；neon-frame-forerun 的姊妹镜
...
## 动效核心
- 金线描框：细金线框（2.5px 金色渐层 + 8px 柔光）从左缘中点两头奔画，~14f 成型；落定后渐淡到 0.35
- 2026-09 黑金改版：原紫橘霓虹渐变框、紫色光晕、灰面板、背景霓虹管框群，改为细金线框、淡金光晕、深色面板（强调块用金）、背景一团极淡金光；镜头弧线与同时贴落不动（使用者从「金线描框＋整页落定 / 光扫揭幕 / 分层浮起 / 浏览器框落定」选了前者）
- 视角弧线：rotateY +38° → -26° 连续弧线（透视原点 30%→64% 随动
  近似相机环绕），贯穿全片
- 同时贴落：全部元素统一 LAND 时刻（归一化 ~0.52）——同帧离地
  悬空、同帧空中下降、同帧贴合；FloatWrap 同形软影（blur/offset/
  opacity 随高度）全体同步收敛
- 背景一团极淡的金色光晕托住页面，主面板落定后让位

...
- 三种框式：`card`（圆角 32 浮卡、左右各离画面 90、柔投影；一般用途预设）、`bleed`（满版出血、上缘与下半压暗、说明 60px 大字压在左下；开场与收尾的冲击镜）、`hairline`（素材几乎占满 SAFE，1.5px 线框内缩 20px + 四角强调色角标）。2026-09 改版：原 `paper` 相纸白边 / `film` 胶卷齿孔读起来复古，已移除
...
| 参数 | 典型值 | 调节手感 |
|------|--------|----------|
| `frameStyle` | 主框：旅游 / 生活 → `card`；财经 / 科技 → `hairline`；开场、收尾要冲击感 → `bleed` | 主框依风格档「影片 / 照片框式」一栏定、全片一种；`bleed` 只额外用在首尾镜 |
...
| card 圆角 / 侧距 | 32 / 90 | 圆角 > 40 会像手机截图；侧距 < 70 投影贴边、读不出「浮」 |
| bleed 压暗 | 上 0.55 → 20% 透；52% 起到 82% 处 0.85 | 下半压暗是为了托住说明与字幕；亮天空素材把说明挪到画面上半、别再加暗 |

## 声音
开框配一记，放在句间气口、音量 ≤ 0.3：`card` → `transition/transition-soft.mp3`；`bleed` → `transition/sweep-short.mp3`（冲击感用 `impact/hit-weak.mp3` 0.25）；`hairline` → `transition/transition-soft.mp3` 或 `ui/switch-light.mp3`。说明进场不配音。素材一律 `muted`，原声不进口播片。
...
- **主框混用（不要）**：框的全部价值是「统一」。card 和 hairline 交替出现，观众会以为框式有语意，然后开始猜不存在的规则。主框全片一种，写进 `theme.ts`；`bleed` 是例外——只用在开场与收尾，位置本身就是它的语意。demo 三种背靠背只是为了 gallery 预览。
...
- **clip-path 会裁掉 box-shadow**：card 的投影不能挂在被裁的那层上，否则开框期间没有影子、开完才突然出现。卡另起一层同宽、高度跟着开口长的影子层（同样圆角）。
